document.addEventListener('DOMContentLoaded', function () {
    ...
    // Add thinking bar element - now we'll add it dynamically when needed
    ...
    function showThinkingBar(action) {
        ...
        // Create thinking bar if it doesn't exist
        if (!thinkingBar) {
            thinkingBar = document.createElement('div');
            thinkingBar.classList.add('thinking-bar');
            
            // Add to chat messages container instead of body
            chatMessages.appendChild(thinkingBar);
        }
        
        // Reset any transition styles that might be applied
        thinkingBar.style.maxHeight = '100px';
        thinkingBar.style.margin = '8px 0 8px 48px';
        thinkingBar.style.padding = '8px 16px';
        
        // Set content
        thinkingBar.textContent = `Thinking: ${action}`;
        thinkingBar.style.opacity = '0';
        thinkingBar.style.display = 'flex';
        
        // Animate in
        setTimeout(() => {
            thinkingBar.style.opacity = '1';
            thinkingBar.style.transform = 'translateY(0)';
            scrollToLastMessage();
        }, 10);
    }
    ...
    function hideThinkingBar() {
        if (!thinkingBar || thinkingBar.style.display === 'none') return;
        
        // Animate out by fading opacity first
        thinkingBar.style.opacity = '0';
        
        // Keep the space occupied until animation completes
        setTimeout(() => {
            // Now smoothly collapse the height
            thinkingBar.style.maxHeight = '0';
            thinkingBar.style.margin = '0';
            thinkingBar.style.padding = '0 16px';
            
            // Finally remove from DOM after all animations complete
            setTimeout(() => {
                if (thinkingBar && thinkingBar.parentNode) {
                    thinkingBar.parentNode.removeChild(thinkingBar);
                }
                thinkingBar = null;
                currentThinkingAction = null;
            }, 300); // Match the transition duration
        }, 200); // Short delay between opacity and height animations
    }
    ...
    async function sendMessage() {
        ...
        if (text && !isOnCooldown) {
            ...
            // Hide example questions if visible
            if (exampleQuestions) {
                exampleQuestions.classList.add('hide-examples');
            }
